...
def test_first_stage(viz, dataloader, net, device, results_dir, criterion=None, isSave=True):
    ...
    return dice_arr,dice_arr_faz,dice_arr_ves
...
def predict_first_stage(dataloader, net, device, results_dir, isSave=True, crop_size=512):
    ...
    with torch.no_grad():
        ...
        for sample in dataloader:
            ...
            pred_faz = np.array(pred_faz)
            disc_pred_faz = pred_faz[0, :, :].squeeze()
            print(disc_pred_faz.max())

            thresh_pred = (disc_pred > 0.5)
            thresh_pred_faz = (disc_pred_faz > 0.5)


            thresh_pred = thresh_pred.astype(np.uint8)
            thresh_pred_faz = thresh_pred_faz.astype(np.uint8)


            thresh_pred_faz = morphology.binary_erosion(thresh_pred_faz, morphology.diamond(5)).astype(np.uint8)  # return 0,1

            thresh_pred_faz = get_largest_fillhole(thresh_pred_faz)

            thresh_pred_faz = morphology.binary_dilation(thresh_pred_faz, morphology.diamond(5)).astype(np.uint8)  # return 0,1

            thresh_pred_faz = get_largest_fillhole(thresh_pred_faz).astype(np.uint8)  # return 0,1
            ...
            thresh_pred = np.array(thresh_pred, np.uint8)*255
            thresh_pred_faz = np.array(thresh_pred_faz, np.uint8)*255
            print(thresh_pred.max())
            print(thresh_pred_faz.max())

            imgName = dataloader.dataset.getFileName()

            ##################220311 resize

            thresh_pred = cv2.resize(thresh_pred, (400, 400))
            thresh_pred_faz = cv2.resize(thresh_pred_faz, (400, 400))
            ...
            if isSave:
                ...
                cv2.imwrite(results_dir + "/ves/" + imgName, thresh_pred)
                cv2.imwrite(results_dir + "/faz/" + imgName, thresh_pred_faz)
