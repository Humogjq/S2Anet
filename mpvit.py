...
class DecoderCup(nn.Module):
    def __init__(self):
        ...
        self.decoder1 = DecoderBlock(64,16)

        self.decoder5_faz = DecoderBlock(512,512)
        self.decoder4_faz = DecoderBlock(512,256)
        self.decoder3_faz = DecoderBlock(256,128)
        self.decoder2_faz = DecoderBlock(128,64)
        self.decoder1_faz = DecoderBlock(64,16)
    ...
    def forward(self, x,d1,d2,d3,d4):
        ...
        x = self.decoder5(x)+d4
        x = self.decoder4(x)+d3
        x = self.decoder3(x)+d2
        x = self.decoder2(x)+d1
        x = self.decoder1(x)

        faz = self.decoder5_faz(faz)+d4
        faz = self.decoder4_faz(faz)+d3
        faz = self.decoder3_faz(faz)+d2
        faz = self.decoder2_faz(faz)+d1
        faz = self.decoder1_faz(faz)
...
class MPViT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        d2 = self.mhca_stages[0](att_inputs)

        att_inputs = self.patch_embed_stages[1](d2)
        d3 = self.mhca_stages[1](att_inputs)

        att_inputs = self.patch_embed_stages[2](d3)
        d4 = self.mhca_stages[2](att_inputs)

        att_inputs = self.patch_embed_stages[3](d4)
        x = self.mhca_stages[3](att_inputs)
    ...
    def forward(self, x):
        ...
        #print(len(x[0]))
        #print(result[0].size())
        out , out_faz = self.decoder(x,d1,d2,d3,d4)
