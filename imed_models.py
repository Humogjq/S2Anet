...
class DACblock(nn.Module):
    def __init__(self, channel):
        ...
        self.ablock = SpatialAttentionBlock(channel)  # 221109 add
    ...
    def forward(self, x):
        ...
        x = self.ablock(x)  # 221109 add
        dilate1_out = self.ablock(dilate1_out)  # 221109 add
        dilate2_out = self.ablock(dilate2_out)  # 221109 add
        dilate3_out = self.ablock(dilate3_out)  # 221109 add
        dilate4_out = self.ablock(dilate4_out)  # 221109 add
...
class SPPblock(nn.Module):
    def __init__(self, in_channels):
        ...
        self.cablock = ChannelAttentionBlock(512)  # 221109 add
    ...
    def forward(self, x):
        ...
        #out = self.cablock(out)  # 221109 add
...
class CE_Net(nn.Module):
    def __init__(self, num_classes=1):
        ...
        #self.ablock = SpatialAttentionBlock(512)    #221109 add
        self.cablock = ChannelAttentionBlock(512)  # 221109 add
...
##########--------- FAZ500net ---------#########
class FAZ500net(nn.Module):
    def __init__(self, num_classes=3):
        super(FAZ500net, self).__init__()
        filters = [64, 128, 256, 512]
        resnet = models.resnet34(pretrained=True)
        self.firstconv = resnet.conv1
        self.firstbn = resnet.bn1
        ...
        self.finalconv2 = nn.Conv2d(32, 32, 3, padding=1)
        self.finalrelu2 = nonlinearity
        self.finalconv3 = nn.Conv2d(32, 1, 3, padding=1)   #export 1 channel
...
##########--------- VES500net ---------#########
class VES500net(nn.Module):
    def __init__(self, num_classes=3):
        super(VES500net, self).__init__()
        filters = [64, 128, 256, 512]
        resnet = models.resnet34(pretrained=True)
        self.firstconv = resnet.conv1
        self.firstbn = resnet.bn1
...
##########--------- VESpial500net ---------#########
class VESpial500net(nn.Module):
    def __init__(self, num_classes=3):
        super(VESpial500net, self).__init__()
        filters = [64, 128, 256, 512]
        resnet = models.resnet34(pretrained=True)
        self.firstconv = resnet.conv1
        self.firstbn = resnet.bn1
        ...
        self.dblock = DACblock(512)
        self.dblock3 = DACblock(256)
        self.dblock2 = DACblock(128)
        self.dblock1 = DACblock(64)


        self.ablock4 = SpatialAttentionBlock(512)    #221109 add
        self.ablock3 = SpatialAttentionBlock(256)  # 221109 add
        self.ablock2 = SpatialAttentionBlock(128)  # 221109 add
        self.ablock1 = SpatialAttentionBlock(64)  # 221109 add
        self.cablock = ChannelAttentionBlock(3)  # 221109 add
    ...
    def forward(self, x):
        ...
        f1 = self.ablock1(f1)
        if (not down_pad) and (not right_pad):
            d4 = self.decoder4(e4) + f3
        elif down_pad and (not right_pad):
            #d4 = self.decoder4(e4)[:, :, :-1, :] + self.ablock3(self.dblock3(e3[:, :, :-1, :]))
            d4 = self.decoder4(e4)[:, :, :-1, :] + f3[:, :, :-1, :]
        elif (not down_pad) and right_pad:
            #d4 = self.decoder4(e4)[:, :, :, :-1] + self.ablock3(self.dblock3(e3[:, :, :, :-1]))
            d4 = self.decoder4(e4)[:, :, :, :-1] + f3[:, :, :, :-1]
        else:
            #d4 = self.decoder4(e4)[:, :, :-1, :-1] + self.ablock3(self.dblock3(e3[:, :, :-1, :-1]))
            d4 = self.decoder4(e4)[:, :, :-1, :-1] + f3[:, :, :-1, :-1]

        #d3 = self.decoder3(d4) + self.ablock2(self.dblock2(e2))
        d3 = self.decoder3(d4) + f2
        #d2 = self.decoder2(d3) + self.ablock1(self.dblock1(e1))
