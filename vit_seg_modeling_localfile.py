...
class Local_Features_cross(nn.Module):
    def __init__(self, config,vis):
        super(Local_Features_cross, self).__init__()
        self.cross_local = Block(config, vis)
        self.cross = Block(config, vis)
        self.cross_b32 = Block(config, vis)
        self.cross_norm = LayerNorm(config.hidden_size, eps=1e-6)
        self.cross_norm_b32 = LayerNorm(config.hidden_size, eps=1e-6)
        #self.len_num = config.patches.grid[0]                              ######################len_num
        #self.cross_norm_1 = LayerNorm(config.hidden_size, eps=1e-6)
        #self.cross_norm_2 = LayerNorm(config.hidden_size, eps=1e-6)

    def forward(self, hidden_states, hidden_states_b32):
        local = hidden_states.clone()
        local_b32 = hidden_states_b32.clone()

        for pos in range(local_b32.size()[1]):
            

            temp = local_b32[:,pos,:].unsqueeze(dim = 1)
            pos_1 = 32*int(pos*2/32)+2*pos          ######################len_num
            #print('###############',pos,'#######',pos_1)

            temp = torch.cat([temp, local[:, pos_1, :].unsqueeze(dim = 1)], dim=1)
            temp = torch.cat([temp, local[:, pos_1+1, :].unsqueeze(dim = 1)], dim=1)
            temp = torch.cat([temp, local[:, pos_1+32, :].unsqueeze(dim = 1)], dim=1)
            temp = torch.cat([temp, local[:, pos_1+33, :].unsqueeze(dim = 1)], dim=1)
            local_crossing, weights = self.cross_local(temp)
            local_b32[:, pos, :] = local_crossing[:,0,:]
            local[:, pos_1, :]   = local_crossing[:,1,:]
            local[:, pos_1+1, :] = local_crossing[:,2,:]
            local[:, pos_1+32,:] = local_crossing[:,3,:]
            local[:, pos_1+33,:] = local_crossing[:,4,:]
        local = self.cross_norm(local)
        local_b32 = self.cross_norm_b32(local_b32)

        local, weights = self.cross(local)
        local = self.cross_norm(local)

        local_b32, weights = self.cross_b32(local_b32)
        local_b32 = self.cross_norm_b32(local_b32)

        return local,local_b32
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=304, num_classes=2, zero_head=False, vis=False):
        ...
        self.Local_Features_cross_0 = Local_Features_cross(config, vis)
        #self.Local_Features_cross_1 = Local_Features_cross(config, vis)
        #self.Local_Features_cross_2 = Local_Features_cross(config, vis)
        #self.Local_Features_cross_3 = Local_Features_cross(config, vis)
        #self.Local_Features_cross_4 = Local_Features_cross(config, vis)
        #self.Local_Features_cross_5 = Local_Features_cross(config, vis)
        ...
        self.Features_cross_6 = Features_cross(config, vis)
        ...
        self.Multioutput_0_local = Multioutput(config)
        self.DecoderCup_b32_0_local = DecoderCup_b32(config)
    ...
    def forward(self, x):
        ...
        local,local_b32 = self.Local_Features_cross_0(x[0],x_b32[0])
        

        
        temp = torch.cat([x[0],x_b32[0]], 1)            #深拷贝还是浅拷贝
        ...
        temp = torch.cat([x[6],x_b32[6]], 1)
        temp = self.Features_cross_6(temp)
        x.append(temp[:,0:x[6].size()[1],:])
        x_b32.append(temp[:,x[6].size()[1]:temp.size()[1],:])
        ...
        result = torch.cat([result, self.DecoderCup_b32_0_cross(x_b32[7],features)], 1)
        result = torch.cat([result, self.Multioutput_0_cross(x[7],features)], 1)

        result = torch.cat([result, self.DecoderCup_b32_0_local(local_b32, features)], 1)
        result = torch.cat([result, self.Multioutput_0_local(local, features)], 1)
        ...
        result_faz = torch.cat([result_faz, self.Multioutput_6_faz(x[6],features)], 1)
        ...
        result_faz = torch.cat([result_faz, self.DecoderCup_b32_faz_6(x_b32[6], features)], 1)

        result_faz = torch.cat([result_faz, self.Multioutput_1_faz_cross(x[8],features)], 1)
        result_faz = torch.cat([result_faz, self.Multioutput_2_faz_cross(x[9],features)], 1)
        result_faz = torch.cat([result_faz, self.Multioutput_3_faz_cross(x[10],features)], 1)
        result_faz = torch.cat([result_faz, self.Multioutput_4_faz_cross(x[11],features)], 1)
        result_faz = torch.cat([result_faz, self.Multioutput_5_faz_cross(x[12],features)], 1)
        result_faz = torch.cat([result_faz, self.Multioutput_6_faz_cross(x[13],features)], 1)

        result_faz = torch.cat([result_faz, self.DecoderCup_b32_faz_1_cross(x_b32[8], features)], 1)
        result_faz = torch.cat([result_faz, self.DecoderCup_b32_faz_2_cross(x_b32[9], features)], 1)
        result_faz = torch.cat([result_faz, self.DecoderCup_b32_faz_3_cross(x_b32[10], features)], 1)
        result_faz = torch.cat([result_faz, self.DecoderCup_b32_faz_4_cross(x_b32[11], features)], 1)
        result_faz = torch.cat([result_faz, self.DecoderCup_b32_faz_5_cross(x_b32[12], features)], 1)
        result_faz = torch.cat([result_faz, self.DecoderCup_b32_faz_6_cross(x_b32[13], features)], 1)
