...
class Features_cross(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states, weights = self.cross(hidden_states)
        encoding = self.encoder_norm(hidden_states)
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=304, num_classes=2, zero_head=False, vis=False):
        ...
        self.Features_cross = Features_cross(config, vis)
        #self.decoder = DecoderCup(config)
        ...
        '''
        self.Multioutput_1 = Multioutput(config)
        self.Multioutput_2 = Multioutput(config)
        self.Multioutput_3 = Multioutput(config)
        self.Multioutput_4 = Multioutput(config)
        self.Multioutput_5 = Multioutput(config)
        self.Multioutput_6 = Multioutput(config)
        self.Multioutput_7 = Multioutput(config)
        self.Multioutput_8 = Multioutput(config)
        self.Multioutput_9 = Multioutput(config)
        self.Multioutput_10 = Multioutput(config)
        self.Multioutput_11= Multioutput(config)
        '''

        #self.Multioutput_0_faz = Multioutput(config)
        ...
        self.Multioutput_7_faz = Multioutput(config)
        self.Multioutput_8_faz = Multioutput(config)
        self.Multioutput_9_faz = Multioutput(config)
        self.Multioutput_10_faz = Multioutput(config)
        self.Multioutput_11_faz = Multioutput(config)
    ...
    def forward(self, x):
        ...
        for lay in range(self.config.transformer.num_layers):
            temp = torch.cat([x[lay],x_b32[lay]], 1)
            temp = self.Features_cross(temp)
            x[lay] = temp[:,0:x[lay].size()[1],:]
            x_b32[lay] = temp[:,x[lay].size()[1]:temp.size()[1],:]
        ...
        #result = torch.cat([result, self.Multioutput_1(x[1],features)], 1)
        #result = torch.cat([result, self.Multioutput_2(x[2],features)], 1)
        #result = torch.cat([result, self.Multioutput_3(x[3],features)], 1)
        #result = torch.cat([result, self.Multioutput_4(x[4],features)], 1)
        #result = torch.cat([result, self.Multioutput_5(x[5],features)], 1)
        #result = torch.cat([result, self.Multioutput_6(x[6],features)], 1)
        #result = torch.cat([result, self.Multioutput_7(x[7],features)], 1)
        #result = torch.cat([result, self.Multioutput_8(x[8],features)], 1)
        #result = torch.cat([result, self.Multioutput_9(x[9],features)], 1)
        #result = torch.cat([result, self.Multioutput_10(x[10],features)], 1)
        #result = torch.cat([result, self.Multioutput_11(x[11],features)], 1)
        #print(result.size())
        logits = self.segmentation_head_2(result)

        
        #result_faz = self.Multioutput_0_faz(x[0],features)
        #result_faz = torch.cat([result_faz, self.Multioutput_1_faz(x[1],features)], 1)
        ...
        #result_faz = self.Multioutput_6_faz(x[6],features)
        #result_faz = torch.cat([result_faz, self.Multioutput_6_faz(x[6],features)], 1)
        #result_faz = self.Multioutput_6_faz(x[6],features)
        #result_faz = torch.cat([result_faz, self.Multioutput_7_faz(x[7],features)], 1)
        #result_faz = torch.cat([result_faz, self.Multioutput_8_faz(x[8],features)], 1)
        #result_faz = torch.cat([result_faz, self.Multioutput_9_faz(x[9],features)], 1)
        #result_faz = torch.cat([result_faz, self.Multioutput_10_faz(x[10],features)], 1)
        #result_faz = torch.cat([result_faz, self.Multioutput_11_faz(x[11],features)], 1)
        
        #print(result_faz.size())
        ...
        for i in range(12):                                                   #12
            branch = self.decoder(x[i], features)
            branch_logits = self.segmentation_head(branch)
            branch_logits_faz = self.segmentation_head_faz(branch)
            if i == 0:
                result = branch_logits
                result_faz = branch_logits_faz
            else:
                result = torch.cat([result, branch_logits], 1)
                result_faz = torch.cat([result_faz,branch_logits_faz], 1)

        
        for i in range(9):                                                   #12
            branch = self.decoder(x[i+3], features)
            branch_logits = self.segmentation_head(branch)
            if i == 0:
                result = branch_logits
            else:
